...
这个目录收拢项目的构建入口脚本。当前统一使用 `build.sh` 作为命令行构建入口，避免 macOS、Android 构建命令分散在不同目录和文档中。
...
脚本现在会在检测到 `rustup` 时优先走 `rustup` toolchain，默认使用 `stable`，避免混用 Homebrew `cargo`/`rustc` 和 `rustup target` 造成 target 不可见的问题。
...
- 在 macOS 上还会继续尝试 `/usr/libexec/java_home -v 17` 和常见的 Homebrew / 系统 JDK 17 安装位置
...
3. 如仓库中还没有 `android/gradlew`，先生成 Wrapper：
...
- 非 Windows 主机交叉构建时：
  ...
  - `x86_64-pc-windows-msvc` 仍可手动指定，但在非 Windows 主机上通常需要你自己提供额外 linker / SDK
...
如果是交叉构建，但脚本未能自动探测 linker，仍然可以手动设置，例如：
...
- 报错 JDK 版本不足：
  需要 JDK 17，低版本 JDK 无法用于当前 Android Gradle Plugin；可通过 `JAVA_HOME` 或 `android/gradle.properties` 的 `org.gradle.java.home` 显式指定
...
- `windows-env` 显示 `WINDOWS_TARGET=<not configured>`：
  说明当前不是 Windows 主机，且脚本也没有自动探测到可用的 GNU Windows 交叉工具链
...
- `windows-release` 报错 linker / C toolchain 问题：
  当前 Windows 构建涉及原生 Win32 和 C 依赖；优先安装 `mingw-w64` 并使用 `x86_64-pc-windows-gnu`，如果仍需特殊 toolchain，再手动设置 target 专属 `LINKER/AR/CC/CXX`
- 明明已经通过 `rustup target add` 安装了 target，但脚本仍提示 target 不存在：
  当前脚本会优先使用 `rustup`；如果你手工执行构建命令，请确认不是落到了 Homebrew 的 `cargo/rustc`
